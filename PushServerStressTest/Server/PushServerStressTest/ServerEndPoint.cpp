...
BOOL CServerEndPoint::SendPushMessage(string strTokenID, string strBody)
{
	...
	TokenId["tokenid"] = Json::Value(strTokenID);
	//TokenId["tokenid"] = Json::Value("safdawer");
	TokenIdArr.append(TokenId);
	TokenBody["bodyID"] =strBody;
	TokenBody["sound"] =Json::Value("default") ;
	TokenBody["alert"] = Json::Value("test1");
	SYSTEMTIME tm;
	GetLocalTime(&tm);
	char szSendTime[64] = {0};
	sprintf_s(szSendTime, _countof(szSendTime), "%02d:%02d:%02d-%03d",
				tm.wHour, tm.wMinute, tm.wSecond, tm.wMilliseconds);
	TokenBody["Time"] = Json::Value(szSendTime);
