...
,m_ClientArr(NULL)
,m_hThreadTimer(NULL)
,m_hThreadConn2PushSrv(NULL)
,m_hThreadDisConn(NULL)
,m_nOnlineNum(0)
//,m_nOnlineCount(0)
...
BOOL CIOCPModel::StartTest(DWORD dwServerIP, UINT nServerPort,
										  ...
										  UINT nClientNum, const TestType nTestType,const TIME_THRESHOLD &tm_shreshold)
{
	m_nOnlineNum = 0;
	m_bIsDeleteAllClient = FALSE;
...
DWORD WINAPI CIOCPModel::_StartTestTrhead(LPVOID lParam)
{
	...
	for (int i=0; i<pThis->m_nClientNum; ++i)
	{
		...
		ClientArr[i].SetTestType(pThis->m_nTestType);
...
DWORD WINAPI CIOCPModel::_Connect2PushSrv(LPVOID lParam)
{
	...
	for (int i=0; i<pThis->m_nClientNum; ++i)
	{
		pConnQueue->Push2Queue(&pThis->m_ClientArr[i]);
		/*if(pThis->m_ClientArr[i].Connect2PushSRV())
		{
			InterlockedIncrement((volatile LONG*)&pThis->m_nOnlineNum);
		}*/
	}
	//DWORD dwSpendTime = GetTickCount() - dwStartTime;
	while( WAIT_OBJECT_0 != ::WaitForSingleObject(pThis->m_hEventStopTest, 0) )
	{
		CClientEndPoint *pClient =NULL;
			pClient = pConnQueue->PopFormQueue();
			if (pThis->m_nTestType == SocketConnect)
			{
				if (NULL != pClient)
				{
