...
/*
#ifdef CLN_UTF8
			m_pDBConnection[i].ExecuteSQL("set names utf8", &SQLRes, iRelt);//设置客户端字符集
#endif
*/
        }
    }
...
int DBSpool::ExecuteSQL(const string& strSQL, MYSQL_RES** res, short& shIdentifier, int& iRelt)
{
    if (m_pDBConnection == NULL){
        gvLog(LOG_ERR_SYS, "(DBSpool::ExecuteSQL) DBConnection is NULL!");
    ...
    while (i == -1){
        ...
        if (m_IdleList.size() < 1){
            ...
            if (iState == 0){
                ...
                gvLog(LOG_ERR_SYS, "(DBSpool::ExecuteSQL) Connect to DB Server wait timeout!");
                return SQL_TYPE_UNKNOW;
            }
        }else{
            i = m_IdleList.front();
    ...
    if (i >= 0){
        ...
        int iSize = 0;
#ifdef DEBUGPROC
        gvLog(LOG_MSG, "execute sql by %d DBConnection...", i+1);
#endif
        switch (iRet){
    ...
    gvLog(LOG_ERR_SYS, "(DBSpool::ExecuteSQL) Connect to DB Server timeout!");
    return SQL_TYPE_UNKNOW;
}
...
bool DBSpool::ReleaseConnection(short shIdentifier)
{
    if (shIdentifier>=1 && m_shCount<=DB_MAX_CONNECT){
        ...
        if (iSize == 1){
            ...
            pthread_cond_broadcast(&m_thCond);
            Unlock(&m_thIdleMutex);
            gvLog(LOG_ERR_SYS, "(DBSpool::ReleaseConnection) send broad cast signal!");
        }
        return true;
    }
    return false;
...
int DBSpool::ExecuteSQL(const string& strSQL, RESDataList* res, int &iRelt)
{
    ...
    if (iRet == SQL_TYPE_SELECT){
        ...
        if(ifields == 0){
            gvLog(LOG_ERR_SYS, "(DBSpool::ExecuteSQL) weird, table fields were returned but no fields in!");
            ReleaseConnection(shIdt);
            return iRet;
        }
...
bool DBConnection::Connect()
{
    ...
    m_pSQLConn = mysql_init((MYSQL*)NULL);
    if(m_pSQLConn == NULL) {
        gvLog(LOG_ERR_SYS, "(DBConnection::SetDBparm) unable to allocate database connection state!");
        return false;
    }
    if (mysql_real_connect(m_pSQLConn, m_strHost.c_str(), m_strUser.c_str(), m_strPasswd.c_str(),
                            m_strDBname.c_str(), m_shPort, NULL, 0) == NULL){
        gvLog(LOG_ERR_SYS, "(DBConnection::SetDBparm) connection to database [%s:%d(%s)] failed: %s", m_strHost.c_str(),
                m_shPort, m_strDBname.c_str(), mysql_error(m_pSQLConn));
        mysql_close(m_pSQLConn);
        m_pSQLConn = NULL;
        return false;
...
#ifdef CLN_UTF8
    if (m_pSQLConn) {
        mysql_query(m_pSQLConn, "set names utf8");//设置客户端字符集
    }
#endif
...
{
    if (m_pSQLConn == NULL){
        gvLog(LOG_ERR_SYS, "(DBConnection::ExecuteSQL) call Connect() at first!");
        return SQL_TYPE_UNKNOW;
    }
    int iTimes = DB_RECONNECT_TIMES;
    bool bTest = true;
    ...
    while (mysql_ping(m_pSQLConn)!=0 && iTimes>0){
        bTest = false;
        gvLog(LOG_ERR_SYS, "(DBConnection::ExecuteSQL) connection to database lost, try to connect remain %d times.\n",  iTimes-1);
        iTimes--;
		if(!bHasUnlock) Unlock();//先解锁，因为Connect中的DisConnect的TestIdle的要等锁
		bHasUnlock = true;
        if (Connect()){
            bTest = true;
            break;
	...
	if(bHasUnlock)Lock();
    if (!bTest){
        gvLog(LOG_ERR_SYS, "(DBConnection::ExecuteSQL) Test connection fail!");
        return SQL_TYPE_UNKNOW;
    }
    if (mysql_query(m_pSQLConn, strSQL.c_str()) != 0){
        gvLog(LOG_ERR_SYS, "(DBConnection::ExecuteSQL) sql execute failed: %s [%s]", mysql_error(m_pSQLConn), strSQL.c_str());
        return SQL_TYPE_UNKNOW;
    }
    ...
    switch (iType){
        case SQL_TYPE_SELECT:
            ...
            if (m_pSQLRes==NULL && mysql_field_count(m_pSQLConn)!=0){
                gvLog(LOG_ERR_SYS, "(DBConnection::ExecuteSQL) sql alloc memory failed: %s", mysql_error(m_pSQLConn));
                return SQL_TYPE_UNKNOW;
            }else{
                *res = m_pSQLRes;
